...
This report outlines the data and methods used to create a music database based on one person's listening history. We called the music database SAAM (Songs Artists Albums & Miscellaneous). SAAM is a data visualization R Shiny app. The main purpose of SAAM is to allow a user to easily search through years of collected listening data of one user and take their own deeper delve into the data. 
